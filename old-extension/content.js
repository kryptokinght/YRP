...
let iframe; let setTimeModal; // iframe stores the main music player
let intervalId;
const video_detail = {
  url: '',
  repeats: 0,
...
*/
chrome.runtime.sendMessage({task: 'checkRefreshState'}, (response) => {
...
function removeMusicPlayer() {
  const temp_iframe = document.getElementById('yrp111'); 
...
function getVideoData() {
  ...
  const videoid = vid[0].baseURI.match(/(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)/);
  const thumbImage =`https://i1.ytimg.com/vi/${videoid[1]}/default.jpg`;
...
function initializeMusicPlayer(save = false, timeData = {}) {
  ...
  chrome.runtime.sendMessage({task: 'searchUrlInStorage', url: video_detail.url}, (response) => {
...
function toggleBrowserAction() {
  ...
  chrome.runtime.sendMessage({task:'getCurrentTabUrl'}, (response) => {
...
function repeatVideo(timeData = {}) {
  const vid = document.getElementsByTagName('video');
  let startTime = 0; let endTime = vid[0].duration - 0.03;
  if(Object.keys(timeData).length) {
